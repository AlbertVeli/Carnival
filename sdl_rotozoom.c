...
int zoomSurfaceRGBA(SDL_Surface * src, SDL_Surface * dst, int flipx, int flipy, int smooth)
{
     ...
     */
    if (unlikely((sax = (int *) malloc((dst->w + 1) * sizeof(Uint32))) == NULL)) {
	return (-1);
    }
    if (unlikely((say = (int *) malloc((dst->h + 1) * sizeof(Uint32))) == NULL)) {
	free(sax);
	return (-1);
    }
    ...
    if (unlikely(flipx)) csp += (src->w-1);
    if (unlikely(flipy)) csp  = (tColorRGBA*)( (Uint8*)csp + src->pitch*(src->h-1) );
...
int zoomSurfaceY(SDL_Surface * src, SDL_Surface * dst)
{
     ...
     * Allocate memory for row increments
     */
    if (unlikely((sax = (Uint32 *) malloc(dst->w * sizeof(Uint32))) == NULL)) {
	return (-1);
    }
    if (unlikely((say = (Uint32 *) malloc(dst->h * sizeof(Uint32))) == NULL)) {
	if (sax != NULL) {
	    free(sax);
	}
	return (-1);
...
void rotozoomSurfaceSizeTrig(int width, int height, double radangle, double zoomx, int *dstwidth, int *dstheight,
...
void rotozoomSurfaceSizeXY(int width, int height, double radangle, double zoomx, int *dstwidth, int *dstheight)
{
    double dummy_sanglezoom, dummy_canglezoom;

    rotozoomSurfaceSizeTrig(width, height, radangle, zoomx, dstwidth, dstheight, &dummy_sanglezoom, &dummy_canglezoom);
}

/* Publically available rotozoom-size function */

void rotozoomSurfaceSize(int width, int height, double radangle, double zoom, int *dstwidth, int *dstheight)
{
    double dummy_sanglezoom, dummy_canglezoom;

    rotozoomSurfaceSizeTrig(width, height, radangle, zoom, dstwidth, dstheight, &dummy_sanglezoom, &dummy_canglezoom);
}

/* Publically available rotozoom function */

SDL_Surface *rotozoomSurface(SDL_Surface * src, double radangle, double zoom, int smooth)
{
  return rotozoomSurfaceXY(src, radangle, zoom, zoom, smooth);
}

/* Publically available rotozoom function */

SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double radangle, double zoomx, double zoomy, int smooth)
{
    SDL_Surface *rz_src;
    SDL_Surface *rz_dst;
    double zoominv;
    ...
    if (likely((is32bit) || (src->format->BitsPerPixel == 8))) {
	/*
     ...
     */
    flipx = (zoomx<0);
    if (unlikely(flipx)) zoomx=-zoomx;
    flipy = (zoomy<0);
    if (unlikely(flipy)) zoomy=-zoomy;
    if (unlikely(zoomx < VALUE_LIMIT)) zoomx = VALUE_LIMIT;
    if (unlikely(zoomy < VALUE_LIMIT)) zoomy = VALUE_LIMIT;
    zoominv = 65536.0 / (zoomx * zoomx);

    /*
     * Check if we have a rotozoom or just a zoom
     */
    if (likely(fabs(radangle) > VALUE_LIMIT)) {
	...
	rotozoomSurfaceSizeTrig(rz_src->w, rz_src->h, radangle, zoomx, &dstwidth, &dstheight, &canglezoom, &sanglezoom);
...
void zoomSurfaceSize(int width, int height, double zoomx, double zoomy, int *dstwidth, int *dstheight)
{
     ...
     */
    if (unlikely(zoomx < VALUE_LIMIT)) {
	zoomx = VALUE_LIMIT;
    }
    if (unlikely(zoomy < VALUE_LIMIT)) {
	zoomy = VALUE_LIMIT;
    }
